...
function love.load(arg)
	...
	wave = new_wave(1)
	add_enemy(200, 10, 1, 100, wave)
	table.insert(waves, wave)
	wave = new_wave(1)
	add_enemy(50, -50, 2, 50, wave)
	table.insert(waves, wave)
...
function love.update(dt)
	...
	if all_enemies_spawned == 1 then
		...
		for _, enemy in pairs(waves[current_wave].enemyList) do
			if (enemy.time == waves[current_wave].time) then
				newEnemy = new_enemy(enemy.x, enemy.y, enemy.img, enemy.type)
...
function draw_player()
	...
	elseif player.velocity.x > 0 then
		love.graphics.draw(player.iH, (player.x - player.img_r:getWidth() / 2) - 4, (player.y - player.img_r:getHeight() /2) + 4)
	else
...
function load_media()
	...
	thickbullet_img = love.graphics.newImage('assets/bullet.png')
	thinbullet_img = love.graphics.newImage('assets/bullet2.png')
