...
function love.load(arg)
	-- Initialize the player's starting position, speed, images and hitbox
		...
		img_l = nil, 
		img_r = nil,
		box = {x = 197, y = 597, h = 5, w = 5}
	}
	debug_mode = 0
	current_wave = 1
	player_bullets = {}
	enemy_bullets = {}
	rotating_enemy_bullets = {}
	enemies = {}
	animations = {}
	waves = {}
	num_enemies = 0
	...
	add_enemy(200, 10, 1, 1000, wave_1)
	--add_enemy(210, 10, 1, 75, wave_1)
	--add_enemy(220, 10, 1, 100, wave_1)
	--add_enemy(230, 10, 1, 125, wave_1)
	--add_enemy(240, 10, 1, 150, wave_1)
	table.insert(waves, wave_1)
	wave_2 = create_wave(1)
	add_enemy(20, -50, 2, 50, wave_2)
	table.insert(waves, wave_2)
	wave_3 = create_wave(1)
	add_enemy(200, -50, 3, 50, wave_3)
	...
	if respawning then
		...
		if respawn_timer == 50 then
			if lives > 0 then
				...
				respawning = false
				animation = new_animation(love.graphics.newImage("assets/exp5.png"), 
										  50, 50, 1, player.x - 100, player.y -100, 32)
...
function love.draw(dt)
	...
	for i, animation in ipairs(animations) do
		local sprite_num = math.floor(animation.current_time / animation.duration * 
									  #animation.quads) + 1
   		love.graphics.draw(animation.sprite_sheet, animation.quads[sprite_num], 
   			               animation.x, animation.y, 0, 4)
...
function draw_player()
	...
	if player.velocity.x < 0 then
		love.graphics.draw(player.img_l, (player.x - player.img_l:getWidth() / 2) + 4, 
					      (player.y - player.img_l:getHeight() /2) + 4)
	elseif player.velocity.x > 0 then
		love.graphics.draw(player.img_r, (player.x - player.img_r:getWidth() / 2) - 4, 
						  (player.y - player.img_r:getHeight() /2) + 4)
	else
		love.graphics.draw(player.img, player.x - player.img:getWidth() / 2, 
						  (player.y - player.img:getHeight() /2) + 4)
	end
...
function draw_rotating_bullets()
...
function load_media()
	...
	fade = love.audio.newSource("fade.wav", "static")
	homing = love.audio.newSource("homing.wav", "static")
end
